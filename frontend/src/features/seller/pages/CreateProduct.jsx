...
import { ArrowLeft, Loader2, AlertCircle, ShoppingBag, Plus, Trash2, X, Upload, Check } from 'lucide-react';
...
const CreateProduct = () => {
    ...
    const [stock, setStock] = useState('0'); // Stock for simple products

    // Variants States
    const [hasVariants, setHasVariants] = useState(false);
    const [options, setOptions] = useState([]); // Array of { name: String, values: [String] }
    const [variants, setVariants] = useState([]); // Array of { attributes: Map, stock: Number, price: Number }
    const [colorFilesMap, setColorFilesMap] = useState({}); // Maps color name -> Array of Files
    ...
    useEffect(() => {
        ...
        // Merge with existing variants to preserve stock/price overrides
        ...
        // Sync colorFilesMap keys with unique colors defined in option "Color"
        const colorOption = options.find(o => o.name.toLowerCase() === 'color');
        if (colorOption) {
            const currentColors = colorOption.values;
            const newColorFilesMap = {};
            currentColors.forEach(c => {
                newColorFilesMap[c] = colorFilesMap[c] || [];
            });
            setColorFilesMap(newColorFilesMap);
        } else {
            setColorFilesMap({});
        }
    }, [options, hasVariants]);
    ...
    const handleColorFilesChange = (color, filesList) => {
        ...
        setColorFilesMap({
            ...colorFilesMap,
            [color]: [...(colorFilesMap[color] || []), ...filesArray]
        });
    };

    // Remove a color file
    const handleRemoveColorFile = (color, fileIdx) => {
        setColorFilesMap({
            ...colorFilesMap,
            [color]: colorFilesMap[color].filter((_, idx) => idx !== fileIdx)
        });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (images.length === 0) {
            setLocalValidationError('Please upload at least one product image (base cover images).');
            return;
        }
        ...
        if (hasVariants) {
            ...
            // Check that stock is a valid positive integer for all variant rows
        ...
        if (hasVariants) {
            ...
            // Compile all files (main images first, then color images)
            const allFiles = [...images];
            const colorFileIndices = {};
            let fileIndex = images.length;

            Object.entries(colorFilesMap).forEach(([color, files]) => {
                if (files && files.length > 0) {
                    colorFileIndices[color] = [];
                    files.forEach(file => {
                        allFiles.push(file);
                        colorFileIndices[color].push(fileIndex);
                        fileIndex++;
                    });
                }
            });
            ...
            const formattedVariants = variants.map(v => {
                const colorKey = Object.keys(v.attributes).find(k => k.toLowerCase() === 'color');
                const colorVal = colorKey ? v.attributes[colorKey] : null;
                const newImageIndices = colorVal ? (colorFileIndices[colorVal] || []) : [];

                return {
                    attributes: v.attributes,
                    stock: Number(v.stock) || 0,
                    price: v.price && v.price !== '' ? { amount: Number(v.price), currency: priceCurrency } : undefined,
                ...
                <div className="mb-8">
                    ...
                    <div className="flex items-center gap-3 border-b border-neutral-200/50 pb-6">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-neutral-500 font-medium">Create a new listing with optional dynamic variant combinations.</p>
                        </div>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="bg-white rounded-3xl border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] p-6 sm:p-8 space-y-6">
                        ...
                        <div className="pt-2 border-t border-neutral-100">
                            <label className="text-xs font-bold uppercase tracking-wider text-gray-400 block mb-2">Base / Cover Images (Fallback)</label>
                            <ImageUploadZone images={images} onChange={setImages} />
                        </div>
                    </div>

                    {/* Variant Section Switcher */}
                    <div className="bg-white rounded-3xl border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] p-6 sm:p-8 space-y-6">
                        ...
                        {!hasVariants ? (
                            ...
                            <div className="space-y-8 animate-in fade-in duration-300">
                                ...
                                {/* Color Images Uploader (conditional on Option: Color) */}
                                {options.some(o => o.name.toLowerCase() === 'color' && o.values.length > 0) && (
                                    <div className="space-y-4 bg-neutral-50/50 p-5 rounded-2xl border border-neutral-100/50">
                                        ...
                                        <p className="text-[10px] text-gray-400 font-medium">Upload images for specific colors once. Sizes of the same color will default to these images automatically.</p>
                                            ...
                                            {options.find(o => o.name.toLowerCase() === 'color').values.map((col, idx) => (
                                                <div key={idx} className="p-4 bg-white border border-neutral-100 rounded-xl space-y-3">
                                                    <div className="flex items-center justify-between border-b border-neutral-50 pb-2">
                                                        ...
                                                        <span className="text-[9px] font-bold text-gray-400 uppercase">Files: {colorFilesMap[col]?.length || 0}</span>
                                                    </div>
                                                    ...
                                                    <div className="flex items-center justify-center border-2 border-dashed border-neutral-200 hover:border-black rounded-xl p-3 bg-neutral-50/35 transition-colors cursor-pointer relative">
                                                        ...
                                                        <div className="text-center space-y-1">
                                                            ...
                                                            <span className="text-[10px] font-semibold text-gray-500 block">Click to upload photos</span>
                                                        </div>
                                                    </div>

                                                    {/* Image thumbnails previews */}
                                                    {colorFilesMap[col] && colorFilesMap[col].length > 0 && (
                                                        <div className="grid grid-cols-4 gap-1.5 pt-1">
                                                            {colorFilesMap[col].map((file, fIdx) => (
                                                                <div key={fIdx} className="relative aspect-square rounded-lg border border-neutral-100 overflow-hidden bg-neutral-50">
                                                                    ...
                                                                    <button
                                                                        type="button"
                                                                        onClick={() => handleRemoveColorFile(col, fIdx)}
                                ...
                                {variants.length > 0 && (
                                    <div className="space-y-4">
                                        ...
                                        <p className="text-[10px] text-gray-400 font-medium">Specify stock count and optional pricing overrides for each active configuration. Uncheck or delete rows to disable unavailable combinations.</p>
